...
class S4DModelForHourlySeries(torch.nn.Module):
    def __init__(self, config: S4DConfig, in_features: int = 15):
        super().__init__()
    ...
    def forward(self, x, mode='recurrent'):
        x = self.input_proj(x)          # (B, 24, hidden_size)
        x = self.s4d(x, mode=mode)      # (B, 24, hidden_size)
