...
class MambaModelForHourlySeries(torch.nn.Module):
    """
    ...
    Shapes:
        ...
        hidden  : (B, 24, config.hidden_size)
        output  : (B, 24)
    """
    def __init__(self, config: MambaConfig, in_features: int = 15):
        super().__init__()

        # 1) feature projection
        self.input_proj = torch.nn.Linear(in_features, config.hidden_size)

        # 2) sequence model
        self.mamba = Mamba(config)

        # 3) output head
        self.output_proj = torch.nn.Linear(config.hidden_size, 1)

    def forward(self, x):
        """
        Args
        ----
        x: Tensor of shape (batch, 24, in_features)
        Returns
        -------
        Tensor of shape (batch, 24)
        """
        x = self.input_proj(x)   # (B, 24, hidden)
        x = self.mamba(x)        # (B, 24, hidden)
        x = self.output_proj(x)  # (B, 24, 1)
